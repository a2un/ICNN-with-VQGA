...
class DecoderRNN(nn.Module):
    ...
    def forward(self, features, captions, lengths):
        ...
        for t in range(self.max_seg_length):
            ...
            packed = pack_padded_sequence(torch.cat([embeddings[:batch_size_t, t, :], attention_weighted_encoding], dim=1), torch.cat((h[:batch_size_t], c[:batch_size_t]),dim=1), batch_first=True,enforce_sorted=False) 
            # torch.cat([embeddings[:batch_size_t, t, :], attention_weighted_encoding], dim=1),
                # (h[:batch_size_t], c[:batch_size_t])
            h, c = self.lstm(packed)  # (batch_size_t, decoder_dim)
