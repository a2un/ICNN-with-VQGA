...
class DecoderRNN(nn.Module):
    ...
    def forward(self, features, captions, lengths):
        ...
        predictions = torch.zeros(features.size(0), max(decode_lengths), vocab_size).to(device)
        alphas = torch.zeros(features.size(0), max(decode_lengths), num_pixels).to(device)
